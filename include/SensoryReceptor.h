...
public:
    explicit SensoryReceptor(const PositionPtr& position) : BodyComponent(position), BodyShapeComponent() {}
    ...
    [[nodiscard]] std::vector<std::shared_ptr<SynapticGap>> getSynapticGaps() const { return synapticGaps; }
