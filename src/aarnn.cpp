/**
 * @file  aarnn.cpp
 * @brief This file contains the implementation of a neuronal network
 * simulation.
...
/*
Setup VTK environment? - note, not required in core aarnn file, only in the
visualiser
 */
...
std::map<std::string, std::string> read_config(const std::vector<std::string>
&filenames) {
 */
...
void checkForQuit()
{
    ...
    while(running)
    {
        ...
        {
            char                  key;
            [[maybe_unused]] auto read__ = read(STDIN_FILENO, &key, 1);
            if(key == 'q')
            {
                running = false;
                break;
...
/**
 * @brief Build a connection string for the database using the configuration
 * map.
 * @param config A map containing the key-value pairs from the configuration
 * files.
...
[[maybe_unused]] static int portaudioMicCallBack(const void                     *inputBuffer,
                                                 void                           *outputBuffer,
 ...
 * @param neuron Pointer to the Neuron object for which the propagation rate is
 * to be calculated.
 */
void computePropagationRate(const std::shared_ptr<Neuron> &neuron)
{
    // Get the propagation rate of the neuron from the DendriteBouton to the
    // SynapticGap
...
void associateSynapticGap(Neuron &neuron1, Neuron &neuron2, double proximityThreshold)
{
    ...
    for(auto &gap: neuron1.getSynapticGapsAxon())
    {
        ...
        if(gap && !gap->isAssociated())
        {
            ...
            {
                std::cout << "Checking gap " << gap->getPosition() << " with dendrite bouton "
                          << dendriteBouton->getPosition() << std::endl;
                // If the distance between the bouton and the dendriteBouton is below
                ...
                if(gap->getPosition()->distanceTo(*(dendriteBouton->getPosition())) < proximityThreshold)
                {
                    ...
                    gap->setAsAssociated();
                    std::cout << "Associated gap " << gap->getPosition() << " with dendrite bouton "
                              << dendriteBouton->getPosition() << std::endl;
    ...
    for(auto &gap: receptor.getSynapticGaps())
    {
        ...
        if(gap && !gap->isAssociated())
        {
            ...
            for(auto &dendriteBouton: neuron.getDendriteBoutons())
            {
                ...
                // If the distance between the gap and the dendriteBouton is below the
                // proximity threshold
                if(gap->getPosition()->distanceTo(*(dendriteBouton->getPosition())) < proximityThreshold)
                {
                    // Associate the synaptic gap with the dendriteBouton
...
/**
 * @brief Initialize the database, checking and creating the required table if
 * needed.
...
void initialize_database(pqxx::connection &conn)
{
    ...
    txn.exec("DROP TABLE IF EXISTS neurons, somas, axonhillocks, axons, "
             "axons_hillock, axonboutons, synapticgaps, dendritebranches_soma, "
             "dendritebranches, dendrites, dendriteboutons, axonbranches "
             "CASCADE; COMMIT;");

    // Check if the "neurons" table exists
    pqxx::result result = txn.exec("SELECT EXISTS ("
                                   "SELECT FROM pg_tables WHERE schemaname = "
                                   "'public' AND tablename = 'neurons'"
                                   ");");

    if(!result.empty() && !result[0][0].as<bool>())
    {
        // Create the "neurons" table if it does not exist
        txn.exec("CREATE TABLE neurons ("
                            ...
                            int                                   &soma_id)
{
    ...
    {
        query = "INSERT INTO dendritebranches_soma (dendrite_branch_id, soma_id, "
                "x, y, z) VALUES ("
    ...
    {
        query = "INSERT INTO dendritebranches (dendrite_branch_id, dendrite_id, x, "
                "y, z) VALUES ("
    ...
    {
        query = "INSERT INTO dendrites (dendrite_id, dendrite_branch_id, x, y, z) "
                "VALUES ("
        ...
        query = "INSERT INTO dendriteboutons (dendrite_bouton_id, dendrite_id, x, y, "
                "z) VALUES ("
...
void insertAxonBranches(pqxx::transaction_base            &txn,
                        ...
                        int                               &axon_hillock_id)
{
    ...
    for(auto &axon: axonBranch->getAxons())
    {
        ...
        query = "INSERT INTO synapticgaps (synaptic_gap_id, axon_bouton_id, x, y, "
                "z) VALUES ("
                   ...
                   int                                                       whichCallBack)
{
    ...
    // vtkSmartPointer<vtkPolyData>::New(); vtkSmartPointer<vtkPolyDataMapper>
...
/**
 * @brief Main function to initialize, run, and finalize the neuron network
 * simulation.
...
int main()
{
    ...
    [[maybe_unused]] bool useDatabase              = convertStringToBool(config["use_database"]);
    ...
    // Currently the application places neurons in a defined pattern but the
    // pattern is not based on cell growth
    for(int i = 0; i < num_neurons; ++i)
    {
        ...
        // std::cout << "Creating neuron " << i << " at (" << shiftX << ", " <<
        // shiftY << ", " << shiftZ << ")" << std::endl;
        ...
        if(i > 0 && i % 3 == 0)
        {
            // std::cout << "Associating neuron " << i << " with neuron " << i - 1 <<
            // std::endl;
            //  First move the required gap closer to the other neuron's dendrite
            //  bouton - also need to adjust other components too
            ...
            // Associate the synaptic gap with the other neuron's dendrite bouton if
    ...
    std::vector<std::vector<std::shared_ptr<SensoryReceptor>>> visualInputs(
     2);  // Resize the vector to contain 2 elements
          // visualInputs[0].reserve(num_pixels / 2);
    std::cout << "Created visual sensory inputs..." << std::endl;
...
#pragma omp parallel for
    for(int j = 0; j < 2; ++j)
    {
        for(int i = 0; i < (num_pixels / 2); ++i)
        {
            ...
            std::cout << "Creating visual (" << j << ") input " << i << " at (" << shiftX << ", " << shiftY << ", "
                      << shiftZ << ")" << std::endl;
    ...
    std::vector<std::vector<std::shared_ptr<SensoryReceptor>>> audioInputs(
     2);  // Resize the vector to contain 2 elements
...
#pragma omp parallel for
    for(int j = 0; j < 2; ++j)
    {
        for(int i = 0; i < (num_phonels / 2); ++i)
        {
            ...
            // std::cout << "Creating auditory (" << j << ") input " << i << " at ("
            // << shiftX << ", " << shiftY << ", " << shiftZ << ")" << std::endl;
    ...
    std::vector<std::vector<std::shared_ptr<SensoryReceptor>>> olfactoryInputs(
     2);  // Resize the vector to contain 2 elements
...
#pragma omp parallel for
    for(int j = 0; j < 2; ++j)
    {
        for(int i = 0; i < (num_scentels / 2); ++i)
        {
            ...
            // std::cout << "Creating olfactory (" << j << ") input " << i << " at ("
            // << shiftX << ", " << shiftY << ", " << shiftZ << ")" << std::endl;
    ...
    std::cout << "Created " << (olfactoryInputs[0].size() + olfactoryInputs[1].size()) << " sensory receptors."
              << std::endl;

    // Create a collection of vocal effector outputs (vocal tract cords/muscle
    // control)
    ...
    for(int i = 0; i < (num_vocels); ++i)
    {
        ...
        // std::cout << "Creating vocal output " << i << " at (" << shiftX << ", "
        // << shiftY << ", " << shiftZ << ")" << std::endl;
...
#pragma omp parallel for schedule(dynamic)
    ...
    // Add any additional functionality for virtual volume constraint and
    // relaxation here Save the propagation rate to the database if it's valid
    // (not null)
    if(propagationRate != 0)
    {
        // std::string query = "INSERT INTO neurons (propagation_rate, neuron_type,
        // axon_length) VALUES (" + std::to_string(propagationRate) + ", 0, 0)";
        // txn.exec(query);
        txn.commit();
    }
    else
