...
void runInteractor(std::vector<std::shared_ptr<Neuron>> &neurons, std::mutex &neuron_mutex, ThreadSafeQueue<std::vector<std::tuple<double, double>>> &audioQueue, int whichCallBack)
{
    ...
    if (whichCallBack == 0)
    {
        ...
        { // or a more appropriate loop condition
            auto executeFunc = [nvCB]() mutable
            { nvCB->Execute(nullptr, vtkCommand::TimerEvent, nullptr); };
    ...
    if (whichCallBack == 1)
    {
        ...
        while (true)
        { // or a more appropriate loop condition
            auto executeFunc = [avCB]() mutable
            { avCB->Execute(nullptr, vtkCommand::TimerEvent, nullptr); };
